...
--0.1  -- check socket data interval
local SOCKET_RECONNECT_TIME = 1 -- socket reconnect try interval
...
function SocketTCP:ctor(__host, __port, __retryConnectWhenFailure)
    ...
    local event = cc.load("event").new()
    event:bind(SocketTCP)
...
function SocketTCP:isIpv6(host)
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
    ...
    gk.log("%s.connect(%s, %d)", self.name, self.host, self.port)
    local ipv6 = SocketTCP:isIpv6(self.host)
...
function SocketTCP:_connectFailure(status)
    gk.log("%s._connectFailure", self.name)
